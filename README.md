...
1. **Clone the repository:**
    ```bash
    git clone https://github.com/gevriel66/InventoryMS.git
    ```
...
3. **Install PHP dependencies:**
    ...
    composer install
    ```
    if there is error Unknown database 'inventoryms' try to run this command:
    ```
    mysql -u root -e "CREATE DATABASE InventoryMS;"
    ```
